...
print(cal[['d', 'date', 'event_name_1', 'event_name_2', 'event_type_1', 'event_type_2', 'snap_CA']].head())
...
# Sales broken down by the time variables
...
# Let's look at a lot of different items
twenty_examples = stv.sample(10, random_state=529)  \
        .set_index('id')[d_cols]    \
        .T  \
...
fig, axs = plt.subplots(5, 2, figsize=(80,20))
axs = axs.flatten()
ax_idx = 0
...
print(stv['cat_id'].unique())

stv.groupby('cat_id').count()['id'] \
    .sort_values() \
    .plot(kind='barh', figsize=(15,5), title='Count of Item by Category')
plt.show()
...
def calmap(ax, year, data):
    ...
    # Month borders
    xticks, labels = [], ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
...
sns.countplot(y="event_name_1", data=cal, ax=ax, palette= "Greens_d", edgecolor="black", linewidth=0.8)
